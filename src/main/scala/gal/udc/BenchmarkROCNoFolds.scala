...
object BenchmarkROCNoFolds
{
  ...
  {
    println("""Usage: BenchmarkROCNoFolds dataset [options]
...
Options:
  ...
  def main(args: Array[String])
  {
        ...
        for (regP <- regPars)
          for (l0 <- lambdas)
                ...
                for (nr <- normalizingRadiuses)
            {
              ...
              val training=trainingDataRDD//.sample(false, 0.55, System.nanoTime().toInt)
              ...
              var modelAUROC = new BinaryClassificationMetrics(probs).areaUnderROC
              ...
              pw.println("\tAUROC:"+modelAUROC+" "+convergenceInfo)
              pw.flush()
              println("\tAUROC:"+modelAUROC+" "+convergenceInfo)
    ...
    admnc.maxIterations=options("max_iterations").asInstanceOf[Double].toInt
    admnc.minibatchSize=options("minibatch").asInstanceOf[Double].toInt
    admnc.regParameter=bestR
    admnc.learningRate0=bestL0
    admnc.learningRateSpeed=bestLS
    admnc.gaussianK=bestG
    admnc.logisticLambda=bestLL
    val training=trainingDataRDD
    admnc.trainWithSGD(sc, training.map(_._1), 0.01)//Anomaly ratio is not important for AUROC
    val test=testDataRDD
    val probs=test.map({case (element, label) =>(admnc.getProbabilityEstimator(element), if (label) 0.0 else 1.0)
                                                               })//Mark as possitive non-anomalous elements, which are given high estimators.
    val testROC = new BinaryClassificationMetrics(probs).areaUnderROC
    
    MLUtils.saveAsLibSVMFile(test.map({ case (element, label) => new LabeledPoint(if (element.isArtificialAnomaly) 1.0 else 0.0, new DenseVector(BDV.vertcat(element.dPart,BDV.vertcat(element.nPart, BDV(admnc.getProbabilityEstimator(element)))).toArray)) }),
                                                  filename.replace("train", "result.libsvm"))
    
    
    pw.println("BEST - R:"+bestR+" L0:"+bestL0+" LS:"+bestLS+" G:"+bestG+" K:"+bestSD+" LL:"+bestLL+" NR:"+bestNR+"\n\tobtained "+testROC+" AUROC ("+bestResult+" AUROC in dev)")
    println("BEST - R:"+bestR+" L0:"+bestL0+" LS:"+bestLS+" G:"+bestG+" K:"+bestSD+" LL:"+bestLL+" NR:"+bestNR+"\n\tobtained "+testROC+" AUROC ("+bestResult+" AUROC in dev)")
    val finishTime=System.currentTimeMillis()
    pw.println("Performed "+totalTests+" tests in "+((finishTime-startTime)/1000.0)+"s ("+((finishTime-startTime)/(1000.0*totalTests))+"s per fold on average)")
    println("Performed "+totalTests+" tests in "+((finishTime-startTime)/1000.0)+"s ("+((finishTime-startTime)/(1000.0*totalTests))+"s per fold on average)")
    pw.close
  }
}